node("build"){
	...
	stage('Go'){
		sh '''#!/bin/bash
			...
			if [ $? != 0 ]; then
				ssh -p 29418 harry-jenkins@review.lineageos.org gerrit review -n OWNER --tag MrRobot --label Code-Review=-1 -m \\'"Validation failed for change $CHANGE, patchset $PATCHSET. View the log at ${BUILD_URL}console"\\' $CHANGE,$PATCHSET
				exit 1
			fi
			echo >> _config.yml
			...
			if [ $? == 0 ]; then
				ssh -p 29418 harry-jenkins@review.lineageos.org gerrit review -n OWNER --tag MrRobot --label Code-Review=+1 -m \\'"Build successful for change $CHANGE, patchset $PATCHSET; validation passed. Preview available at https://harryyoud.co.uk/lineage-previews/${PRIVATE}${CHANGE}/${PATCHSET}"\\' $CHANGE,$PATCHSET
			else
